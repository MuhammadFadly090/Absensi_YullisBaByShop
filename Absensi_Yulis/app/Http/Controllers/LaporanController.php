...
class LaporanController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        $request->validate([
            ...
            'absen' => 'nullable|in:Masuk,Pulang', // Tambahkan validasi untuk kolom 'absen'
        ]);
        ...
        $absensi = Absen::with('karyawan')
            ...
            ->when($absen, function ($query) use ($absen) {
                $absenValue = $absen === 'Masuk' ? 0 : 1;
                $query->where('absen', $absenValue);
            })
            ->orderBy('tanggal', 'desc')
            ->get();
    ...
    public function download(Request $request)
    {
        $absensi = Absen::with('karyawan')
            ...
            ->when($request->filled('absen'), function ($query) use ($request) {
                $absenValue = $request->absen === 'Masuk' ? 0 : 1;
                $query->where('absen', $absenValue);
            })
            ->orderBy('tanggal', 'desc')
            ->get();
