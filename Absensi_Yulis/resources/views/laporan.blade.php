        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('laporan.filter') }}" method="post">
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                ...
                                <div>
                                    <label for="absen" class="block text-sm font-medium text-gray-700">Absen:</label>
                                    <select name="absen" id="absen" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        <option value="">Semua</option>
                                        <option value="Masuk" {{ request()->get('absen') == 'Masuk' ? 'selected' : '' }}>Masuk</option>
                                        <option value="Pulang" {{ request()->get('absen') == 'Pulang' ? 'selected' : '' }}>Pulang</option>
                                    </select>
                                </div>
                                <div class="flex items-end justify-end space-x-2">
                                    ...
                                    <a href="{{ route('laporan.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        Refresh
                                    </a>
                                    <a href="{{ route('laporan.download', request()->all()) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        Download
                                    </a>
                                </div>
                            </div>
                    ...
                    <table class="min-w-full border-collapse table-auto">
                        <thead>
                            <tr>
                                ...
                                <th class="border border-gray-400 px-4 py-2">Absen</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border border-gray-400 px-4 py-2">{{ $absen->absen ? 'Pulang' : 'Masuk' }}</td>
                                <td class="border border-gray-400 px-4 py-2">
                                    <form action="{{ route('absen.delete', $absen->id_karyawan) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Anda yakin ingin menghapus absensi?')" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">
                                            Hapus
                                        </button>
                                    </form>
                                </td>
                    ...
                    @else
                    <p class="text-center mt-4">Tidak ada data absensi yang tersedia.</p>
                    @endif
                </div>
            </div>
        </div>
