                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="p-3 border border-gray-300">Keterangan</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="p-3 border border-gray-300">{{ $item->aktif ? 'Aktif' : 'Tidak Aktif' }}</td>
                                <td class="p-3 border border-gray-300">
                                    <button class="bg-red-500 text-white px-4 py-2 rounded nonaktifkan-karyawan" data-id="{{ $item->id_karyawan }}">Nonaktifkan Karyawan</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize Select2 for tempat_lahir field
        $('#tempat_lahir').select2({
            placeholder: "Pilih Kota",
            allowClear: true
        });

        // Handler for nonaktifkan-karyawan button click
        $('.nonaktifkan-karyawan').on('click', function() {
            var karyawanId = $(this).data('id');
            var button = $(this);

            $.ajax({
                url: '/karyawan/nonaktifkan/' + karyawanId,
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(response) {
                    if (response.success) {
                        button.closest('tr').find('td:eq(6)').text('Tidak Aktif'); // Mengubah teks status menjadi "Tidak Aktif"
                        button.text('Karyawan Dinonaktifkan');
